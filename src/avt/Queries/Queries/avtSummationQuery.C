...
avtSummationQuery::PostExecute(void)
{
    ...
    // Gather the global count of contributing (non-ghost) elements. Used for
    // count-based averages and for transparency in the result message.
    double globalCount = 0.;
    SumDoubleArrayAcrossAllProcessors(&elementCount, &globalCount, 1);
    elementCount = globalCount;

    if (CalculateAverage())
    {
        if (AverageByCount())
        {
            // Unweighted arithmetic mean: divide by the number of elements.
            if (elementCount != 0.)
            {
                for(int i=0; i < ncomps; i++)
                    sums[i] /= elementCount;
        ...
        {
            // Size-weighted mean: divide by the total weight.
            double newDenomSum;
    ...
    char buf[1024];
    string str;
    string weightDescriptor = GetAverageWeightingDescriptor();
    if (CalculateAverage())
    {
        if (AverageByCount())
        {
            // Unweighted arithmetic mean (VisIt issue #19952).
            str += "The average (unweighted) value of ";
        }
        else if (!weightDescriptor.empty())
        {
            // Be explicit that this is a size-weighted mean, not the simple
            // arithmetic mean (VisIt issue #19952).
            str += "The " + weightDescriptor + "-weighted average value of ";
        }
        else
        {
            str += "The weighted average value of ";
        }
    }
    else
        str += "The total ";
    ...
    // For averages, make the computation transparent: report how many
    // (non-ghost) elements were used and, for a weighted average, the total
    // weight and how to obtain the unweighted arithmetic mean instead.
    // See VisIt issue #19952.
    if (CalculateAverage())
    {
        result_node["count"] = elementCount;
        if (AverageByCount())
        {
            snprintf(buf, 1024, "\n(arithmetic mean of %g non-ghost values)",
                     elementCount);
            str += buf;
        }
        else
        {
            result_node["total_weight"] = denomSum;
            if (!weightDescriptor.empty())
                snprintf(buf, 1024,
                    "\n(weighted by %s; %g non-ghost values, total weight = %g)"
                    "\nUse the \"Average Value (Unweighted)\" query for the "
                    "arithmetic mean.",
                    weightDescriptor.c_str(), elementCount, denomSum);
            else
                snprintf(buf, 1024,
                    "\n(%g non-ghost values, total weight = %g)"
                    "\nUse the \"Average Value (Unweighted)\" query for the "
                    "arithmetic mean.",
                    elementCount, denomSum);
            str += buf;
        }
    }
...
avtSummationQuery::Execute(vtkDataSet *ds, const int dom)
{
    ...
    bool averageByCount = AverageByCount();
    // A weight-based average needs the per-element weight array (denominator).
    // A count-based (unweighted) average just counts contributing elements, so
    // it does not need a denominator array.
    if (doAverage && !averageByCount)
    {
        if (pointData)
    ...
    for (int i = 0 ; i < nvalues ; i++)
    {
        ...
        for(int j = 0; j < ncomps; j++)
            sums[j] += vals[j];
        // Count every non-ghost element that contributes to the sum. This is
        // used both for the unweighted (count-based) average and for reporting
        // how many elements went into a (weighted) average.
        elementCount += 1.0;
        if (doAverage && !averageByCount)
