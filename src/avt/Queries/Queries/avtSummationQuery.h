...
class QUERY_API avtSummationQuery : public avtDatasetQuery
{
  ...
  protected:
    ...
    // When CalculateAverage() is true, controls how the average is normalized:
    //   false (default) -> divide the (weighted) sum by the total weight
    //                      (denomVariableName), i.e. a size-weighted mean.
    //   true            -> divide the sum of the variable by the number of
    //                      non-ghost elements, i.e. the unweighted arithmetic
    //                      mean. See VisIt issue #19952.
    virtual bool                    AverageByCount() { return false; };

    // Human-readable name of the weighting basis used for a weighted average
    // ("length", "area", "revolved-volume", "volume", or "" if not applicable).
    // Used only to make the result message transparent about what was computed.
    virtual std::string             GetAverageWeightingDescriptor()
                                             { return ""; };
};
