...
#include <avtAverageValueUnweightedQuery.h>
...
avtQueryFactory::CreateQuery(const QueryAttributes *qa)
{
    ...
    else if (CaseInsenstiveEqual(qname,"Average Value (Unweighted)"))
    {
        query = new avtAverageValueUnweightedQuery();
    }
    ...
    else if (CaseInsenstiveEqual(qname,"Statistics"))
    {
        query = new avtStatisticsQuery();
    }
