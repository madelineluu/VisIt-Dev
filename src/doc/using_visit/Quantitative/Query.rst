...
Average Value
    The Average Value query computes a **cell-size-weighted** mean of the
    variable used by the plot highlighted in the **Plot list**, that is

    .. math::

        \frac{\sum_i v_i\, w_i}{\sum_i w_i}

    where :math:`v_i` is the variable value of element :math:`i` and
    :math:`w_i` is its geometric size (length in 1D, area in 2D-XY, revolved
    volume in 2D-RZ, volume in 3D, and :math:`1` for point meshes). Ghost
    elements are excluded. Because larger cells count more, this is **not** the
    same as the simple arithmetic mean unless all cells are the same size. The
    result message states the weighting basis, the number of (non-ghost)
    elements, and the total weight. To get the simple arithmetic mean instead,
    use *Average Value (Unweighted)*.

Average Value (Unweighted)
    Computes the **unweighted** arithmetic mean of the variable,
    :math:`\frac{1}{N}\sum_i v_i`, over the :math:`N` non-ghost elements. This
    is the value most users expect from an "average". For meshes whose cells
    all have the same size it equals *Average Value*.
...
Statistics
    The Statistics query reports several descriptive statistics for the
    variable used by the plot highlighted in the **Plot list** in a single
    pass: the element count, minimum, maximum, range, sum, (unweighted)
    arithmetic mean, standard deviation, variance, skewness, and kurtosis. All
    measures are returned in the result so that scripts can read any of them
    with :py:func:`GetQueryOutputValue` / :py:func:`GetQueryOutputObject`.
    Ghost elements are excluded. The mean reported here is unweighted; use
    *Average Value* for the cell-size-weighted mean.
