...
// ****************************************************************************
//  Function: SlurpAndDecodeCurveStream
//
//  Purpose:
//      Reads the entire input stream into a std::string, detecting any leading
//      byte-order mark (BOM). If a UTF-8 BOM is found it is stripped. If a
//      UTF-16 LE or UTF-16 BE BOM is found, the data is transcoded into 8-bit
//      ASCII by keeping the low byte of every 16-bit code unit (sufficient for
//      the ASCII numerics, whitespace, comments and curve names that the
//      Curve2D format supports). Non-ASCII code units are replaced with a
//      space so the existing parser can still scan past them.
//
//      This addresses VisIt issue #18516 where curve/ultra files saved by
//      PowerShell's `cat` (UTF-16 LE) could not be opened.
//
//  Programmer: Madeline Luu
//  Creation:   June 1, 2026
//
// ****************************************************************************
static string
SlurpAndDecodeCurveStream(std::istream &in)
{
    std::ostringstream raw;
    raw << in.rdbuf();
    string bytes = raw.str();
    ...
    {
        debug3 << "avtCurve2DFileFormat: detected UTF-8 BOM, stripping."
               << endl;
        return bytes.substr(3);
    }

    bool utf16le = bytes.size() >= 2 &&
                   (unsigned char)bytes[0] == 0xFF &&
                   (unsigned char)bytes[1] == 0xFE;
    bool utf16be = bytes.size() >= 2 &&
                   (unsigned char)bytes[0] == 0xFE &&
                   (unsigned char)bytes[1] == 0xFF;

    if (utf16le || utf16be)
    {
        debug3 << "avtCurve2DFileFormat: detected UTF-16 "
               << (utf16le ? "LE" : "BE")
               << " BOM, transcoding to ASCII." << endl;
        size_t lowOff  = utf16le ? 0 : 1;
        size_t highOff = utf16le ? 1 : 0;
        string out;
        out.reserve((bytes.size() - 2) / 2 + 1);
        for (size_t i = 2; i + 1 < bytes.size(); i += 2)
        {
            unsigned char lo = (unsigned char) bytes[i + lowOff];
            unsigned char hi = (unsigned char) bytes[i + highOff];
            if (hi == 0 && lo < 0x80)
...
//
//    Madeline Luu, Sun Jun  1 00:00:00 PDT 2026
//    Fix for issue #18516: detect and handle UTF-8 / UTF-16 LE / UTF-16 BE
//    byte-order marks so curve / ultra files saved with those encodings
//    (e.g. PowerShell's `cat`) open correctly.
// ****************************************************************************
...
avtCurve2DFileFormat::ReadFile(void)
{
    ...
    //
    // Slurp the file contents and strip / transcode any BOM up front so that
    // UTF-8-with-BOM and UTF-16 LE/BE files (e.g. those produced by
    // PowerShell's `cat`) parse correctly. See issue #18516.
    //
    string decoded = SlurpAndDecodeCurveStream(ifile());
    std::istringstream parseStream(decoded);
    ...
    while (!parseStream.eof())
    {
        double   x, y;
        string  lineName;
        CurveToken t = GetPoint(parseStream, x, y, lineName);
        switch (t)
        {
          case VALID_POINT:
          {
