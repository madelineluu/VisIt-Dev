...
// X11 includes disabled for headless build
#endif
...
#if defined(Q_OS_LINUX)
    //
    // Headless build - no X11, use defaults
    //
...
//
// Headless stub - X11 disabled for headless build
//
...
WindowMetrics::CalculateBorders(QWidget *,
    int &borderT, int &borderB, int &borderL, int &borderR)
{
    // Return constructor defaults - no X11 available
    borderT = 26; borderB = 4; borderL = 4; borderR = 4;
}
...
#if 0
// X11 code disabled for headless build - kept for reference
